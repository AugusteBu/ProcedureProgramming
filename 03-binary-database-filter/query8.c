...
int main(int argc, char *argv[])
{
    ...
    if (db)
    {
        ...
        int rez1, rez2, calc_and_alg;       // students who study both Calculus and Algebra
        ...
        for (int i = 0; i < size; ++i)  // process all the student records in database
        {
            ...
            {
                rez1 = strcmp(s.courses[j], "Algebra");
                rez2 = strcmp(s.courses[j], "Calculus"); //checks if a student has both calculus and algebra
                if(rez1 == 0 || rez2 == 0)
                    calc_and_alg++;
            }
            if(calc_and_alg == 2)  // *** first filter, conditions on the student
            {
