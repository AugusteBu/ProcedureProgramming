...
int main(int argc, char *argv[])
{
    ...
    if (db)
    {
        ...
        for (int i = 0; i < size; ++i)  // process all the student records in database
        {
            ...
            bool philosophy_rezults = 0;
            for(int j = 0; j < s.load; ++j)
            {
                if(strcmp(s.courses[j], "Philosophy") == 0 && s.grades[j] > 4)
                {
                    philosophy_rezults = 1;
                    break;
                }
            }

            if(philosophy_rezults == 1)  // *** first filter, conditions on the student
            {
